...
func (h *RequestHandler) convertBMSDataToRegisters(
	...
	setReg(0, data.CircuitBreakerStatus)
	setReg(1, uint16(data.Voltage*10))
	setReg(2, uint16(data.Current*10))
	setReg(3, data.SOC)
	setReg(4, data.SOH)
	setReg(5, uint16(data.MaxCellVoltage*1000))
	setReg(6, data.MaxVoltageRackNo)
	setReg(7, data.MaxVoltageCellNo)
	setReg(8, uint16(data.MinCellVoltage*1000))
	setReg(9, data.MinVoltageRackNo)
	setReg(10, data.MinVoltageCellNo)
	setReg(11, uint16(data.MaxCellTemperature))
	setReg(12, data.MaxTempRackNo)
	setReg(13, data.MaxTempCellNo)
	setReg(14, uint16(data.MinCellTemperature))
	setReg(15, data.MinTempRackNo)
	setReg(16, data.MinTempCellNo)
	set32BitReg(setReg, 17, uint32(data.TotalChargeEnergy*10))
	set32BitReg(setReg, 19, uint32(data.TotalDischargeEnergy*10))
	set32BitReg(setReg, 21, uint32(data.ChargeCapacity*10))
	set32BitReg(setReg, 23, uint32(data.DischargeCapacity*10))
	setReg(25, data.AvailableDischargeTime)
	setReg(26, data.AvailableChargeTime)
	setReg(27, uint16(data.MaxDischargePower*10))
	setReg(28, uint16(data.MaxChargePower*10))
	setReg(29, uint16(data.MaxDischargeCurrent*10))
	setReg(30, uint16(data.MaxChargeCurrent*10))
	setReg(31, data.DischargeTimesToday)
	setReg(32, data.ChargeTimesToday)
	set32BitReg(setReg, 33, uint32(data.DischargeEnergyToday*10))
	set32BitReg(setReg, 35, uint32(data.ChargeEnergyToday*10))
	setReg(37, uint16(data.Temperature))
	setReg(38, data.State)
	setReg(39, data.ChargeDischargeState)
	setReg(40, data.InsulationResistance)
