...
func NewManager(configs []config.BMSConfig, influxDB *database.InfluxDB, alarmManager *alarm.Manager) *Manager {
	managerLogger := logger.With(logger.String("component", "bms_manager"))
...
func (m *Manager) Start() error {
	...
	for id, service := range m.services {
		if err := service.Start(); err != nil {
			m.log.Error("Failed to start BMS service", logger.Int("id", id), logger.Err(err))
			return fmt.Errorf("failed to start BMS service %d: %w", id, err)
		}
		m.log.Info("BMS service started", logger.Int("id", id))
	}
...
func (m *Manager) Stop() {
	...
	for id, service := range m.services {
		service.Stop()
		m.log.Info("BMS service stopped", logger.Int("id", id))
	}
}
